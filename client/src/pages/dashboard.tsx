...
import { LogOut, Wifi, WifiOff, Fingerprint, Bell, Share, X, BookOpen, BarChart2, Bot, Video, ClipboardList, Inbox, Users, Menu, Globe, BookUser, ContactRound } from "lucide-react";
            ...
            {isAdmin && (
              <Link href="/customers">
                <a title={t("customers")} className="flex items-center gap-1.5 text-xs text-white/70 hover:text-white transition-colors p-2 xl:px-3 xl:py-1.5 rounded-md hover:bg-white/10">
                  <ContactRound className="w-4 h-4 shrink-0" /><span className="hidden xl:inline">{t("customers")}</span>
                </a>
              </Link>
            )}
            ...
            <nav className="flex-1 overflow-y-auto py-2">
              {(
                [
                  ...
                  ...(isAdmin ? [{ href: "/customers", icon: <ContactRound className="w-5 h-5" />, label: t("customers") }] : []),
